...
REAL :: ZDENSITY, ZCONVERTFACM0,  ZCONVERTFACM3, ZCONVERTFACM6, ZINIRADIUS, ZINISIG
...
#ifdef MNH_FOREFIRE
IF (LFOREFIRE) ZP_SFTS(:,NSV_DSTBEG:NSV_DSTEND) = 0. ! pour enlever les flux net de dust de surfex (pour FF)
#endif
!
#ifdef CPLOASIS
IF (LOASIS) THEN
...
DO JMODE=1,NMODE_DST
  ...
  IF (CRGUNITD=="MASS") THEN
    ZINIRADIUS = XINIRADIUS(JPDUSTORDER(JMODE)) * EXP(-3.*(LOG(ZINISIG))**2)
  ELSE
    ZINIRADIUS = XINIRADIUS(JPDUSTORDER(JMODE))
  END IF
 ...
 !Get flux of number in #/m2/sec from flux of mass in kg/m2/sec
  ZFM(:,:,1) = ZSFTS(:,:,NSV_DSTBEG+JSV_IDX+1)                     & ! kg_{dst}/m2/sec
             / ZINIRADIUS**3                & ! *um^{-3} ==> #/m2/sec*(m3/um3)
